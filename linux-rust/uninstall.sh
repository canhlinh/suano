...
BINDING_PATH="/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/aihelper/"

# Stop running instance
pkill -f aihelper 2>/dev/null || true

# Remove binary
rm -f "$HOME/.local/bin/aihelper"

# Remove desktop entries
rm -f "$HOME/.local/share/applications/$APP_ID.desktop"
rm -f "$HOME/.local/share/applications/aihelper.desktop"
rm -f "$HOME/.config/autostart/aihelper.desktop"
...
for size in 16x16 32x32 128x128 256x256 512x512; do
    rm -f "$HOME/.local/share/icons/hicolor/$size/apps/aihelper.png"
done
gtk-update-icon-cache -f "$HOME/.local/share/icons/hicolor" 2>/dev/null || true

# Remove config
rm -rf "$HOME/.config/aihelper"
